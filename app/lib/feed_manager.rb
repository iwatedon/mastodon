...
class FeedManager
  ...
  def unpush_from_home(account, status, update: false)
    ...
    unless update
      redis.publish("timeline:#{account.id}", { event: :delete, payload: status.id.to_s }.to_json)
      redis.publish("timeline:#{account.id}:media", { event: :delete, payload: status.id.to_s }.to_json) if status.proper.media_attachments.any?
    end
    true
  end
